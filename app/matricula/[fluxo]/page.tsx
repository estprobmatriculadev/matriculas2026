...
export default function MatriculaPage() {
  ...
  const fluxo = params.fluxo as "EP" | "PEDFOR";
  ...
  const carregarDados = async (email: string) => {
    try {
      ...
      // No mundo real, um cursista pode ter múltiplos vínculos
      const vinculos = processarPerfilCursista([docSnap.data()]);
      setPerfil(vinculos);

      // 2. Filtrar Turmas baseadas no fluxo e perfil
      ...
      const turmasFiltradas = todasTurmas.filter(t => {
        if (fluxo === "EP") {
          // Regras EP: Mesma modalidade e Ano Formativo
        ...
        return true; // PEDFOR é mais flexível
      });
  ...
  if (protocolo) return (
    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass p-12 rounded-3xl text-center max-w-lg">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="p-6 rounded-2xl bg-amber-500/5 border border-amber-500/20 text-amber-200/70 text-xs leading-relaxed">
            <p><strong>Atenção:</strong> As turmas exibidas são filtradas automaticamente de acordo com sua compatibilidade de horário e perfil funcional.</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          ...
          {turmas.length === 0 ? (
            <div className="glass p-12 rounded-3xl text-center text-white/40">
              Nenhuma turma disponível para o seu perfil no momento.
            </div>
          ) : (
            <div className="space-y-4">
              {turmas.map(t => (
